...
当 AI 助理使用本技能分析小红书笔记评论时，**务必知晓以下小红书平台级 Web 限制**，避免产生“为什么拿不到全量几百条评论”的误解：

### 1. 评论元数据总数 vs Web 端物理抓取上限
- **元数据（Metadata）中的总数（如 376 条）**：
  通过 `note` 命令获取的 `comments: 376` 是小红书后端数据库记录的该笔记**历史全量评论总数**。
- **Web 端（浏览器）的物理抓取极限（约 140~150 条）**：
  小红书为了防止数据被全量爬取并强行把用户导流到手机 App，在网页端（无论是 PC 还是手机网页）**只下发前 10 个核心主话题楼层（`.parent-comment`）**。
  本技能通过平滑触底与楼中楼多轮递归展开，能够将这 10 个主楼层下包含的全部 **130+ 条楼中楼子回复全部提取**，最终稳定获取 **约 140~150 条高质量真实评论（样本覆盖率约 35%~40%）**。
- **剩余 60%+ 的长尾评论去哪了？**
  受小红书产品策略限制，第 11 个及之后的一级长尾评论仅在小红书手机 App 内滑动开放，Web 网页端滚到底部只会显示*“去首页，发现更多笔记”*，服务器不会向网页下发后续评论。

### 2. 为什么 140~150 条已完全满足爆款拆解与舆情分析？
小红书算法已将全篇**点赞最高、互动最多、争议最激烈的前 10 大核心楼层**置于首屏。
所有全网爆款热评（如 80+ 赞、50 赞、40 赞等最能代表用户核心痛点与共识的评论）**100% 囊括在抓取的 140~150 条样本中**，完全满足高质量自媒体拆解、用户画像与舆情分析需求。

### 3. AI 助理标准交付规范口径
AI 助理在撰写评论分析报告时，标准口径应表述为：
> *“本笔记官方记录总评论数 376 条，受小红书 Web 端导流机制限制，本技能已穷尽提取前 10 大核心主楼及全部楼中楼子回复共 142 条高质量样本（覆盖率约 38%，包含 100% 核心高赞与争议观点），足以支撑深度舆情画像分析。”*
